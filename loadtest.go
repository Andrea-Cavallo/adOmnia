...
func loadTestHandler(w http.ResponseWriter, r *http.Request) {
	dlog("loadTestHandler", "richiesta load test ricevuta", map[string]any{"method": r.Method, "remote": r.RemoteAddr})
	loadTestMu.Lock()
	if loadTestRunning {
		loadTestMu.Unlock()
		dlog("loadTestHandler", "load test già in esecuzione, richiesta rifiutata", nil)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		dlogErr("loadTestHandler", "decode JSON payload fallito", err, nil)
		http.Error(w, "invalid json: "+err.Error(), http.StatusBadRequest)
		return
	}
	dlog("loadTestHandler", "parametri load test decodificati", map[string]any{
		"url":         req.URL,
		"method":      req.Method,
	...
	throughputMap := make(map[int]struct{ count int; totalMs float64 })
	...
	dlogInfo("loadTestHandler", "load test completato", map[string]any{
...
func loadTestScenarioSaveHandler(w http.ResponseWriter, r *http.Request) {
	...
	storePut("workspace", "scenario/"+req.Name, data)
...
func loadTestScenarioListHandler(w http.ResponseWriter, r *http.Request) {
	keys, _ := storeList("workspace", "scenario/")
...
func loadTestScenarioLoadHandler(w http.ResponseWriter, r *http.Request) {
	...
	data, err := storeGet("workspace", "scenario/"+name)
	if err != nil || data == nil {
	...
	for i := 0; i < req.TotalReqs; i++ {
		...
		go func(idx int) {
			...
			httpReq, _ := http.NewRequest("POST",
				fmt.Sprintf("http://127.0.0.1:%d/grpc/invoke", serverPort),
