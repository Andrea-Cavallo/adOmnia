...
func caGenerateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := generateCACert(); err != nil {
		jsonError(w, "failed to generate CA: "+err.Error(), 500)
		return
	}
...
func caExportHandler(w http.ResponseWriter, r *http.Request) {
	...
	if cert == nil {
		jsonError(w, "no CA certificate available", 404)
		return
	}
	...
	case "crt", "der":
		...
		if block == nil {
			jsonError(w, "failed to decode CA cert", 500)
			return
		}
		content = block.Bytes
	...
	default:
		jsonError(w, "unsupported format: "+format+". Use pem or crt", 400)
		return
	}
