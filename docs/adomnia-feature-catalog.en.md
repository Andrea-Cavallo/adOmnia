...
| A5.17 | **Stress Test** | The **Stress** dock runs the whole flow with up to 25 isolated virtual users. Smoke/load/spike/soak presets join custom ramp-up, iterations/duration and think time. Results include overall and per-step p50/p90/p95/p99, throughput/data rate, status and normalized error groups, explicit p95/error/RPS release gates, deterministic bottleneck/tail/regression guidance, baseline comparison, and CSV/JSON/HTML evidence. Latency is the Go HTTP executor time; step time also includes app overhead. Results are session-only. |
