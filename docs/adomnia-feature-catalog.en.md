...
| A5.17 | **Stress Test** | The **Stress** toolbar button opens a dock that runs the whole flow with up to 25 concurrent virtual users: ramp-up, total iterations or fixed duration, think time, Stop. Each user keeps its own variables. Live progress, req/s, errors, per-step p50/p90/p95/p99/max and a throughput sparkline; export a per-request CSV log, a JSON run file and a self-contained HTML report through the native save dialog. **Compare with baseline** loads a previous JSON export and shows the per-step p95 change and the throughput change. Latency is the HTTP time measured by the Go executor; step time also includes app overhead. Results are session-only. |
