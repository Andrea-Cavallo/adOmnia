...
func dnsLookupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Host == "" {
		jsonError(w, "host required", http.StatusBadRequest)
		return
	}
	if req.Type == "" {
		req.Type = "A"
	...
	if !ok {
		jsonError(w, "unsupported DNS type: "+req.Type, http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		jsonError(w, "dns lookup failed: "+err.Error(), http.StatusOK)
		return
	}
