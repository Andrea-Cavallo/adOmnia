...
const KEY = 'adomnia.bughunt.preferences.arcade.v3'
...
export function loadPreferences(): BugHuntPreferences {
  ...
  try {
    ...
    const legacy = campaign ? null : JSON.parse(localStorage.getItem('adomnia.bughunt.preferences.campaign.v2') ?? localStorage.getItem('adomnia.bughunt.preferences.v1') ?? 'null')
