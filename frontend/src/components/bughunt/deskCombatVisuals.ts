...
export function drawDeskArenas(ctx: CanvasRenderingContext2D, state: VisualState) {
  ...
  for (const b of state.enemies) {
    ...
    if (c.phase === 'tell' || c.phase === 'attack') {
      ...
      } else if ((b.encounter === 'retry' || b.encounter === 'legacy') && c.move === 0) {
        ctx.beginPath(); ctx.moveTo(c.startX + b.w / 2, 450)
        ctx.quadraticCurveTo((c.startX + c.targetX) / 2 + b.w / 2, b.encounter === 'legacy' ? 100 : c.enraged ? 200 : 250, c.targetX + b.w / 2, 450); ctx.stroke()
      }
      ctx.setLineDash([])
    }
    if (c.phase === 'recover') {
