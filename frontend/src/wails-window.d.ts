...
interface WailsGoMain {
  ...
  GitSync: {
    ...
    GetRepoState: (repoPath: string) => Promise<string>
    GetCommitMeta: (repoPath: string, ref: string) => Promise<string>
    RemoteWebURL: (repoPath: string) => Promise<string>
    CompareWebURL: (repoPath: string, refA: string, refB: string) => Promise<string>
    CheckoutCommit: (repoPath: string, ref: string, newBranch: string) => Promise<string>
    CreateBranchFromCommit: (repoPath: string, branch: string, ref: string, checkout: boolean, push: boolean) => Promise<string>
    CreateTagFromCommit: (repoPath: string, name: string, ref: string, message: string, annotated: boolean, push: boolean) => Promise<string>
    CompareCommits: (repoPath: string, refA: string, refB: string) => Promise<string>
    CreatePatch: (repoPath: string, refA: string, refB: string) => Promise<string>
    ApplyPatch: (repoPath: string, patch: string, threeWay: boolean, index: boolean) => Promise<string>
    RestoreFileFromCommit: (repoPath: string, ref: string, path: string) => Promise<string>
    FileAtCommit: (repoPath: string, ref: string, path: string) => Promise<string>
    FileHistory: (repoPath: string, path: string, n: number) => Promise<string>
    BlameFile: (repoPath: string, path: string) => Promise<string>
    CherryPick: (repoPath: string, shas: string[], noCommit: boolean, newBranch: string, recordOrigin: boolean) => Promise<string>
    GenerateRevertMessage: (repoPath: string, sha: string) => Promise<string>
    RevertCommit: (repoPath: string, sha: string, message: string, mainline: number) => Promise<string>
    ResetBranch: (repoPath: string, sha: string, mode: string) => Promise<string>
    AmendCommit: (repoPath: string, message: string, addAll: boolean) => Promise<string>
    UndoLastCommit: (repoPath: string, keepStaged: boolean) => Promise<string>
    SquashHeadIntoPrevious: (repoPath: string) => Promise<string>
    ExtractHeadToNewBranch: (repoPath: string, newBranch: string) => Promise<string>
    ForcePush: (repoPath: string, branch: string) => Promise<string>
    ContinueOperation: (repoPath: string) => Promise<string>
    SkipOperation: (repoPath: string) => Promise<string>
    AbortOperation: (repoPath: string) => Promise<string>
    GetRebaseTodo: (repoPath: string, baseRef: string) => Promise<string>
    StartInteractiveRebase: (repoPath: string, baseRef: string, itemsJSON: string) => Promise<string>
    RebaseOnto: (repoPath: string, targetRef: string) => Promise<string>
    BisectStart: (repoPath: string, bad: string, good: string) => Promise<string>
    BisectMark: (repoPath: string, verdict: string) => Promise<string>
