...
var (
	...
	persistRules  func([]byte) error
)

// Configure provides local persistence dependencies used by proxy state and CA material.
func Configure(dataDir string, persist func([]byte) error) {
...
func proxyStartHandler(w http.ResponseWriter, r *http.Request) {
	devlog.Log("proxyStartHandler", "richiesta avvio proxy intercettore ricevuta", map[string]any{"remote": r.RemoteAddr})
	...
	if err := json.NewDecoder(r.Body).Decode(&cfg); err != nil {
		devlog.Err("proxyStartHandler", "decode configurazione proxy fallito", err, nil)
	...
	devlog.Log("proxyStartHandler", "configurazione proxy decodificata", map[string]any{"port": cfg.Port, "enableHTTPS": cfg.EnableHTTPS})
	...
	devlog.Info("proxyStartHandler", "proxy intercettore avviato", map[string]any{"port": cfg.Port, "https": cfg.EnableHTTPS})
...
func proxyStopHandler(w http.ResponseWriter, r *http.Request) {
	devlog.Log("proxyStopHandler", "richiesta arresto proxy intercettore", map[string]any{"remote": r.RemoteAddr})
	if r.Method != http.MethodPost {
		http.Error(w, "POST only", http.StatusMethodNotAllowed)
		return
	}
	...
	if interceptProxy != nil {
		...
		interceptProxy = nil
		devlog.Info("proxyStopHandler", "proxy intercettore fermato", nil)
	} else {
		devlog.Log("proxyStopHandler", "nessun proxy attivo da fermare", nil)
	}
...
func proxyBreakpointResumeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if body.ID == "" {
		httputil.JSONError(w, "id is required", 400)
		return
	}
	if body.Action == "" {
		body.Action = "resume"
	...
	pendingBreakpointsMu.Unlock()
	if pending == nil {
		httputil.JSONError(w, "pending breakpoint not found", 404)
		return
	}
	...
	case <-time.After(2 * time.Second):
		httputil.JSONError(w, "request is no longer waiting", 409)
	}
}
...
// RegisterHandlers registers proxy interception, mapping, rule and CA endpoints.
