...
// Configure supplies the local application directory used for Python plugins.
func Configure(dataDir string) {
	dataDirectory = dataDir
}

// ConfigureSDKDir overrides SDK discovery for hosts that run away from the app directory.
func ConfigureSDKDir(dir string) {
	sdkDirectory = dir
}
...
// Init stores the context for deferred initialization.
// No network listeners or subprocesses are started here.
func (pb *PythonBridge) Init(ctx context.Context) {
	pb.ctx = ctx
