...
func workspaceSaveHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := Put("workspace", "v2", []byte(req.State)); err != nil {
		http.Error(w, "save failed", http.StatusInternalServerError)
		return
	}
	stateKey := fmt.Sprintf("ws-state/%s", req.Name)
	if err := Put("workspace", stateKey, []byte(req.State)); err != nil {
...
func workspaceLoadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if name := r.URL.Query().Get("name"); name != "" {
		data, _ = Get("workspace", fmt.Sprintf("ws-state/%s", name))
	}
	if data == nil {
		data, _ = Get("workspace", "v2")
	}
	w.Header().Set("Content-Type", "application/json")
	if data == nil {
		w.Write([]byte("{}"))
