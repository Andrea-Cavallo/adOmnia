...
func (tm *ThemeManager) GetSkinsDirectory() string {
	return filepath.Join(storage.DataDir(), "skins")
}
...
func (tm *ThemeManager) GetExtendedBuiltinThemes() []Theme {
	return []Theme{
		...
		{
			ID:          "builtin-terminal-green",
			Name:        "Terminal Green",
			Author:      "adOmnia",
			Version:     "1.0.0",
			Description: "Near-black developer workspace with a single signal-green accent and a matching adOmnia mark",
			Colors: map[string]string{
				"surface-0":      "#030906",
				"surface-1":      "#07100A",
				"surface-2":      "#0A1710",
				...
				"accent-light":   "#B7FFD0",
				"accent-dark":    "#24B865",
				"accent-glow":    "rgba(100, 243, 154, 0.20)",
				"success":        "#64F39A",
				"warning":        "#F5C451",
				"error":          "#FF6F75",
				"info":           "#65D9E8",
				...
				"json-bool":      "#77ECA7",
				"json-null":      "#92A99A",
				"json-bracket-1": "#E8FFF1",
				"json-bracket-2": "#91F7B7",
				"json-bracket-3": "#85E7F4",
			},
			Fonts: ThemeFonts{
			...
			Radii:   map[string]string{},
			Shadows: map[string]string{},
			// The token palette carries the usable UI; the scoped CSS treatment
			// recolours only adOmnia artwork and preserves all user content.
			Meta: map[string]string{"builtin": "true", "style": "terminal", "skin": "terminal-green", "mode": "dark"},
		},
		{
			ID:          "builtin-sketch",
			Name:        "Sketch",
