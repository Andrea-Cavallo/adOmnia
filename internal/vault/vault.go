...
func TimeoutMinutes() int {
	return int(vaultTimeout.Minutes())
}

// SetTimeoutMinutes updates the inactivity timeout used while unlocked.
...
func vaultImportHandler(w http.ResponseWriter, r *http.Request) {
	...
	imported := 0
	_ = storage.DB().Update(func(tx *bolt.Tx) error {
...
// RegisterHandlers registers local secret vault endpoints.
